...
namespace ly::communicating {
    ...
    struct typed_message {
        ...
        struct formatter : const_byte_span_formatter {
            auto format(const typed_message& message, std::format_context& ctx) const {
                return const_byte_span_formatter::format(static_cast<const_byte_span>(message), ctx);
            }
        };
    };
